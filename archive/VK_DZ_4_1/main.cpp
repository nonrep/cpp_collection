...
// структура для хранения номера массива, элемент из которого будет извелечен с помощью метода ExtractMax
template<class T>
struct Pair {
    T data;
    int arrayNumber;
...
class IsLessDefault {
public:
    bool operator()(const Pair& l, const Pair& r) { return (l.data < r.data); }
};
...
template<class T, class IsLess>
void Heap<T, IsLess>::Insert(T data) {
...
T* heapSort(const T* const * const & arrays, T* sizes, const int& k, int numberOfElements) {
    ...
    for (int i = 0; i < k; i++) {
            if (sizes[i] > 0) {
                heap.Insert(Pair(arrays[i][sizes[i] - 1], i));
                --sizes[i];
            }
        }

    // освободить 1 место в куче
    sortedArray[numberOfElements - 1] = heap.ExtractMax();
    --numberOfElements;
    ...
    while (numberOfElements > k) {
        for (int i = 0; i < k; i++) {
            if (sizes[i] > 0) {
                heap.Insert(arrays[i][sizes[i] - 1]);
                sortedArray[numberOfElements - 1] = heap.ExtractMax();
                --numberOfElements;
                --sizes[i];
            }
    ...
    // вставка оставшихся элементов из кучи
